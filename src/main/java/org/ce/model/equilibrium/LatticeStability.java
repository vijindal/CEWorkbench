 ...
 * of {@code G = G0m + Gm} in the reference Hillert-method implementation
 * this package ports.
 *
 ...
 *
 * <p>Standard SGTE/Dinsdale-style unary lattice-stability polynomials, one
 * piecewise function per (element, structure), ported literally from a
 * working Mathematica reference implementation. Structures are defined
 * either directly ({@code ghserXX}, the structure's own reference) or as a
 * constant/linear-in-T offset from another structure's {@code ghserXX}
 * (e.g. {@code gfccMo = 15200 + 0.63*T + ghserMo}). Coefficients and
 * temperature-range breakpoints are reproduced exactly as given; do not
 * "clean up" or refit them.</p>
 *
 * <p>Element/structure coverage matches what the reference defines — not
 * every structure exists for every element (e.g. vanadium has no FCC_A1
 * entry). Requesting an undefined (element, phase) pair is a programming
 * error, not a silently-zero fallback: {@link #g0}/{@link #g0m} throw
 * rather than guess.</p>
 *
 * <p><b>Every derivative in this class is analytical</b> (term-by-term
 * differentiation of the same closed-form polynomials {@code g0} uses),
 * never finite-differenced — required throughout the Hillert solver
 * (HILLERT_SOLVER_PLAN.md). Each {@code dXxxDt} method mirrors its {@code
 * Xxx} counterpart's exact piecewise branch structure, so a derivative is
 * always evaluated against the same formula {@code g0} would have used at
 * that {@code T}.</p>
 */
public final class LatticeStability {
    ...
    /**
     * Composition-weighted pure-element reference energy for a phase --
     * port of the reference's {@code calG0m}: {@code G0m = Σ x_i * G0(i)}.
     * No mixing physics here; that is entirely {@code Gm} (the CVM
     * evaluator's {@code calculateG()}).
     *
     * @param elements    canonical element order, length K
     * @param phase       structure name (e.g. "BCC_A2")
     * @param composition mole fractions in the same order as elements, length K
     ...
     * differentiation via {@link #dG0Dt}, same composition weights {@link
     * #g0m} uses) -- feeds {@code GT}'s pure-element contribution in the
     * Hillert solver.
     */
