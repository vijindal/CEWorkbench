 ...
 *
 * <p>
 * Produced by the model layer (optimizers emit progress during long
 * computations).
 * Consumed by the calculation and UI layers to drive live feedback (charts,
 * progress bars).
 * Text log messages continue to flow through the separate
 * {@code Consumer<String>} progressSink.
 * </p>
 */
public abstract class ProgressEvent {

    /**
     * Signals the start of a new calculation — listeners should clear prior chart
     * data.
     */
     ...
     *
     * <p>
     * {@code cfs} contains the running-average correlation functions computed by
     * {@code MCSampler.meanCFs()} during the AVERAGING phase; it is {@code null}
     * during EQUILIBRATION (sampler has not yet accumulated data).
     * </p>
     */
    ...
    /**
     * One Newton-Raphson iteration — emitted by CVM post-hoc from the stored
     * iteration
     * trace after convergence.
     *
     * <p>
     * Note: on solver failure {@code CVMGibbsModel.evaluate()} throws before the
     * trace
     * is readable, so chart events are only emitted on successful convergence.
     * </p>
     */
    public static final class CvmIteration extends ProgressEvent {
        ...
        /**
         * Non-point correlation functions at this iteration (length = ncf); may be
         * null.
         */
        public final double[] cfs;
