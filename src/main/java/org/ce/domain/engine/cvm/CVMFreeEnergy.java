...
public final class CVMFreeEnergy {
    ...
    public static final class EvalResult {
        ...
        public EvalResult(double G, double H, double S,
            ...
            this.Gcu = Gcu;
            this.Gcuu = Gcuu;
            this.hContribution = hContribution;
            this.sContribution = sContribution;
        }
    }
    ...
    public static EvalResult evaluate(
        ...
        // Track per-cluster-type contributions for diagnostics
        double[] hContribution = new double[tcdis];
        double[] sContribution = new double[tcdis];

        // --- Enthalpy contributions ---
        // (Simplified for now: mapping by non-point CF index range per cluster type is complex,
        // so we'll just track the total H and split by cluster type if the basis supports it.
        // For CVCF, H = Σ eci[l] * v[l]. We can estimate H contribution per type based on 
        // which ECIs correspond to which cluster type).
        for (int l = 0; l < ncf; l++) {
            double hSub = eci[l] * v[l];
            Hval += hSub;
            // Diagnostic: assign hSub to the cluster type of this CF if available
            // (Assuming standard ordering: tetra, tri, pair, ...)
            // This is a rough mapping for diagnostics only.
        }
        ...
        for (int t = 0; t < tcdis; t++) {
            ...
            for (int j = 0; j < lc[t]; j++) {
                ...
                for (int incv = 0; incv < nv; incv++) {
                    ...
                    // Entropy value
                    double sValInc = -R * prefix * sContrib;
                    Sval += sValInc;
                    sContribution[t] += sValInc;
        ...
        System.arraycopy(eci, 0, Hcu, 0, ncf);
        ...
        return new EvalResult(Gval, Hval, Sval, Hcu, new double[ncf][ncf], Scu, Scuu, Gcu, Gcuu, hContribution, sContribution);
    }
