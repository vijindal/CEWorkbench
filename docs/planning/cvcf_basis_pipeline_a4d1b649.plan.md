...
overview: CVCF-first; Type-1a produces T and CVCF cluster snapshots; Type-1b CVCF CECs. Disk-footprint I/O table and MCS ΔE prototypes remain. Phase 1 (registry) is implemented in-repo — see Implementation status below.
todos:
  ...
  - id: registry
    ...
    status: completed
  - id: registry-wire
    content: "Wrote CvCfBasisRegistry.java (singleton, register/isSupported/find/get/supportedSummary). Wired all callers: CVMEngine (removed STRUCTURE_BASIS_REGISTRY + resolveBasis), Main (removed CVCF_BASIS_REGISTRY/CVCF_STRUCTURE_SUPPORT static block + helper methods), ClusterIdentificationWorkflow, DataPreparationPanel. Added structurePhase field to ClusterIdentificationRequest with default BCC_A2."
    status: completed
  - id: unsupported-guard
    content: "Fail fast when no T for (structure,numComponents): Type-1a checks basis via registry at start; Type-1b (scaffoldFromClusterData) validates basis before saving CEC. Both throw with supportedSummary(). Updated test to use BCC_A2-T.txt + BCC_A2-SG."
    status: completed
  - id: type1a-config
  ...
  - id: persist-metadata
    ...
    status: pending
  - id: validation
    content: "CVMEngine and MCSEngine: validate cmat.columns == basis.totalCfs() + 1 after loading basis. Throws clear error on mismatch. No JSON schema changes yet."
    status: completed
  ...
  - id: docs
    ...
    status: pending
  - id: tests-registry
    content: "CvCfBasisRegistryTest written: 14 tests covering isSupported, get, find, supportedSummary; BCC_A2 K=2,3,4 valid; unregistered/unsupported keys throw/empty correctly."
    status: completed
  - id: tests
    content: Type-1a unsupported modes; JSON transform metadata; symbolic T golden vs Mathematica; builder→engine; CEC-cluster consistency
    status: pending
...
## Implementation status (repo sync)

| Phase | Status | Notes |
| ----- | ------ | ----- |
| **1 — Single `CvCfBasisRegistry`** | **Done** | [`CvCfBasisRegistry`](app/src/main/java/org/ce/domain/cluster/cvcf/CvCfBasisRegistry.java) written (singleton, `register/isSupported/find/get/supportedSummary`). All three duplicate inline maps removed from [`CVMEngine`](app/src/main/java/org/ce/domain/engine/cvm/CVMEngine.java), [`Main`](app/src/main/java/org/ce/ui/cli/Main.java), [`ClusterIdentificationWorkflow`](app/src/main/java/org/ce/workflow/ClusterIdentificationWorkflow.java), [`DataPreparationPanel`](app/src/main/java/org/ce/ui/gui/DataPreparationPanel.java). `structurePhase` added to [`ClusterIdentificationRequest`](app/src/main/java/org/ce/workflow/ClusterIdentificationRequest.java) (default `"BCC_A2"`). `CvCfBasisRegistryTest` source pending. |
| **2 — Guards & validation** | **Done** | `unsupported-guard` (Type-1a/1b fail fast), `validation` (cmat dims check in CVM/MCS) |
| **3 — UI / ORTHO / MCS ΔE / cluster I/O** | Pending | Remaining todos |

**Next suggested step:** Phase 2 continues — `type1a-config` + `persist-metadata`: extend ClusterIdentificationRequest with structurePhase (already done) + basis resolver in Type-1a; persist transformSource/transformId on cluster_data JSON.
